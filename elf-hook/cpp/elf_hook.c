...
#define LOG_TAG "vipkid_elf_hook"
...
static int locate_sysbol_linear(const loaded_soinfo *soinfo,const char *name, Elf_Sym **sys_out, size_t *sys_idx_out);

static int locate_sysbol_hash(const loaded_soinfo *soinfo, const char *name, Elf_Sym **sys_out, size_t *sys_idx_out);
